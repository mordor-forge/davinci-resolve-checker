...
class TestRenderJson:
    def test_valid_json_output(self, capsys, nvidia_desktop) -> None:
        results = [
    ...
    def test_system_state_serialized(self, capsys, nvidia_desktop) -> None:
        render_json(nvidia_desktop, [])
        captured = capsys.readouterr()
        data = json.loads(captured.out)
        assert data["system"]["distro_id"] == "arch"

    def test_failed_json_output_sets_ok_false(self, capsys, nvidia_desktop) -> None:
...
class TestRenderText:
    def test_renders_without_error(self, capsys, nvidia_desktop) -> None:
        results = [
            CheckResult(status=CheckStatus.PASS, message="All good"),
    ...
    def test_shows_gpu_info(self, capsys, nvidia_desktop) -> None:
        render_text(nvidia_desktop, [])
        captured = capsys.readouterr()
        assert "RTX 2070" in captured.out

    def test_shows_distro(self, capsys, nvidia_desktop) -> None:
        render_text(nvidia_desktop, [])
        captured = capsys.readouterr()
